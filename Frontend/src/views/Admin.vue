...
const loadAllUsers = async () => {
  ...
  try {
    ...
    if (result.success && result.data) {
      users.value = result.data.users;  // 페이징된 사용자 목록
  ...
  try {
    ...
    if (searchType.value === 'all') {
      ...
      if (!searchKeyword.value.trim()) {
        ...
        url = `http://localhost:8080/uhpooh/api/user/search/admin?isAdmin=${searchKeyword.value === '관리자' ? '1' : '0'}&page=${currentPage.value}&size=${pageSize.value}&requestUserId=${requestUserId}`;
      }
    } else {
      if (!searchKeyword.value.trim()) {
        url = `http://localhost:8080/uhpooh/api/user/list?page=${currentPage.value}&size=${pageSize.value}&requestUserId=${requestUserId}`;
      ...
      <div class="bg-white rounded-2xl shadow-sm">
        ...
        <div v-else class="overflow-x-auto">
          <table class="w-full min-w-max">
            <thead>
              <tr class="bg-gray-50">
                <th class="p-4 text-left text-sm font-medium text-gray-600">프로필</th>
                <th class="p-4 text-left text-sm font-medium text-gray-600">ID</th>
                <th class="p-4 text-left text-sm font-medium text-gray-600">이름</th>
                <th class="p-4 text-left text-sm font-medium text-gray-600">이메일</th>
                <th class="p-4 text-left text-sm font-medium text-gray-600">주소</th>
                <th class="p-4 text-left text-sm font-medium text-gray-600">권한</th>
                <th class="p-4 text-left text-sm font-medium text-gray-600">로그인 상태</th>
                <th class="p-4 text-left text-sm font-medium text-gray-600">등록일</th>
              </tr>
            </thead>
            ...
            <tbody class="divide-y">
              <tr v-for="user in users" :key="user.userId" class="hover:bg-gray-50">
                ...
                <td class="p-4">
                  <img
                    :src="user.pimage || 'default-profile.png'"
                    alt="Profile"
                ...
                <td class="p-4">
                  <span 
                    :class="user.isAdmin === 1 ? 'bg-red-100 text-red-600' : 'bg-blue-100 text-blue-600'"
                ...
                <td class="p-4">
                  <span 
                    :class="user.isLogin === 1 ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-600'"
                    class="px-2 py-1 rounded-full text-sm"
                  >
