<template>
  <div class="min-h-screen bg-sky-50">
    <!-- Header Section -->
    <div
      class="relative bg-gradient-to-r from-cyan-600 to-blue-400 px-8 pt-16 pb-24 text-white overflow-hidden"
    >
      <div class="max-w-6xl mx-auto text-center relative z-10">
        ...
        <div class="flex justify-center gap-8 mt-8">
          <div
            class="px-8 py-4 rounded-2xl bg-white/10 backdrop-blur border border-white/20"
          >
      ...
      <section class="bg-white rounded-3xl shadow-lg p-8 mb-8 relative">
        <div class="flex justify-between items-center mb-8">
          <div class="flex items-center gap-4">
            ...
            <span
              class="px-4 py-1 bg-gradient-to-r from-cyan-600 to-blue-400 text-white rounded-full text-sm font-medium"
            >
              {{ likedPlacesLength }}개
            </span>
          </div>
        ...
        <div v-else-if="likedPlacesLength > 0" class="relative group">
          ...
          <div class="overflow-hidden px-4">
            <div
              ...
              :style="{
                transform: `translateX(-${currentLikedPage * (300 + 24)}px)`,
              }"
            >
              <div
              ...
              >
                <div class="relative h-48 overflow-hidden">
                  ...
                  />
                  <div
                    class="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-center pb-6"
                  >
                    <span
                      class="text-white font-medium px-6 py-2 rounded-full bg-white/20 backdrop-blur-sm"
                    >
                      자세히 보기
                    </span>
                  </div>
        ...
        <div
          v-else
          class="text-center py-16 border-2 border-dashed border-gray-200 rounded-2xl"
        >
          <img
      ...
      <section class="bg-white rounded-3xl shadow-lg p-8 mb-8">
        ...
        <div v-else-if="myReviewsLength > 0">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            >
              <div
                class="flex flex-col gap-2 mb-4 pb-4 border-b border-gray-100"
              >
                <div class="flex justify-between items-start">
                  ...
                  <span class="text-sm text-cyan-600">{{
                    formatDate(review.regTime)
                  }}</span>
                </div>
                <span class="text-sm text-cyan-600"
                  >🏊‍♂️ {{ review.placeName }}</span
                >
              </div>
...
const totalPages = computed(() =>
  Math.ceil(myReviews.value.length / itemsPerPage)
);
...
const nextLikedPage = () => {
  ...
  if (nextScroll < maxLikedScroll.value) {
    ...
    maxLikedScroll.value >
    currentLikedPage.value * (cardWidth + cardGap)
  ) {
    // 마지막 스크롤 위치로 이동
    currentLikedPage.value = Math.ceil(
      maxLikedScroll.value / (cardWidth + cardGap)
    );
  }
};
...
const goPlaceBoard = async (placeId) => {
  const response = await axios.get(
    import.meta.env.VITE_API_URL + "/place/getplacebyplaceid/" + placeId
  );
        ...
        response.data.data.items.map(async (review) => {
          try {
            const placeResponse = await axios.get(
              `${import.meta.env.VITE_API_URL}/place/getplacebyplaceid/${
                review.placeId
              }`
            );
            return {
