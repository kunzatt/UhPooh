...
// 토스페이먼츠 객체 초기화
const tossPayments = window.TossPayments(import.meta.env.VITE_TOSS_CLIENT_KEY); // 여기에 클라이언트 키를 넣으세요

// 결제 완료 후 서버에 예약 정보 저장
const saveReservation = async (paymentData) => {
  try {
    const response = await axios.post(
      API_ENDPOINTS.PAYMENTS.RESERVATIONS,
      {
...
const handlePayment = async () => {
  try {
    ...
    await tossPayments.requestPayment("카드", {
      ...
      successUrl: API_ENDPOINTS.PAYMENTS.SUCCESS, // 결제 성공 시 리다이렉트될 URL
      failUrl: API_ENDPOINTS.PAYMENTS.FAIL, // 결제 실패 시 리다이렉트될 URL,
    });
  } catch (error) {
