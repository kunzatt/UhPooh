...
public class OAuth2Controller {
  ...
  @GetMapping("/success")
  public void success(HttpServletResponse response, @AuthenticationPrincipal OAuth2User oauth2User)
      throws IOException {
    ...
    try {
      ...
      // 프론트엔드로 리다이렉트
      String redirectUrl = UriComponentsBuilder.fromUriString(frontendUrl).path("/oauth2/callback")
          .queryParam("token", token).queryParam("userId", user.getUserId())
          .queryParam("email", user.getUserEmail()).queryParam("name", user.getUserName()).build()
          .toUriString();
